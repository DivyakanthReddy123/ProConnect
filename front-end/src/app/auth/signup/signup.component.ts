...
export class SignupComponent implements OnInit {
  ...
  redirect(userDetails: UserDetails): string{
    ...
    if(this.hasRole('ADMIN', userDetails))
      redirectUrl = '/admin';
    else if(this.hasRole('RECRUITER', userDetails))
      redirectUrl = '/recruiter/dashboard';
    else if(this.hasRole('PROFESSIONAL', userDetails)) 
      redirectUrl = '/feed';
  ...
  signup(signupForm) {
    if (signupForm.form.valid  && (this.user.password === this.user.passwordConfirm)) {
      this.user.signupRole = this.signupRole;
