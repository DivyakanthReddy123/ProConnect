    ...
    public ResponseEntity<?> signup(@RequestPart("object") User user, @RequestPart(value = "imageFile", required = false) MultipartFile file) throws IOException {
        if(userRepository.findUserByUsername(user.getUsername()) == null) {
            if (user.getPassword().equals(user.getPasswordConfirm())) {
                ...
                RoleType selectedRole = RoleType.PROFESSIONAL;
                if (user.getSignupRole() != null && user.getSignupRole().equalsIgnoreCase("RECRUITER")) {
                    selectedRole = RoleType.RECRUITER;
                }
                Role r = roleRepository.findByName(selectedRole);
                if (r == null) {
                    r = roleRepository.findByName(RoleType.PROFESSIONAL);
                }
                roles.add(r);
                user.setRoles(roles);
                if(file!=null){
